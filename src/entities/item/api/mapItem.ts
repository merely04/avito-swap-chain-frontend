...
/** Тем же « или », которым варианты склеиваются перед отправкой, они и разбираются обратно. */
const splitWish = (stored: string | undefined) =>
  (stored ?? '')
    .split(' или ')
 ...
 * - **желание в контракте одной строкой.** Разбираем её обратно в варианты по нашему же
 *   разделителю: наша модель шире, и когда бэк примет список, маппер станет тривиальным.
 */
export const mapItem = (item: ApiItem): Item => ({
  ...
  // Контракт хранит желание одной строкой, а у нас это список вариантов — при отправке
  // они склеиваются через « или » (`asWantDescription`), здесь разбираются обратно.
  // Без этого правка объявления открывалась с одним слипшимся полем вместо трёх строк,
  // и человек терял варианты, которые указал. Разделитель наш собственный, поэтому
  // разбор симметричен склейке; уйдёт вместе с `wantDescriptions` в контракте.
  wish: splitWish(item.wantDescription),
