...
describe('selectView — цепочка formed, вид зависит от моего ответа', () => {
  ...
  // Отказ распускает цепочку, поэтому в норме такой пары статусов не бывает. Вид всё равно
  // свой: показывать отказавшемуся предложение с кнопкой «подтвердить» нельзя ни при каких данных.
